...
class AutoAttendanceCommand extends Command
{
    ...
    public function handle()
    {
        ...
        // 1. Get all employers who do NOT have auto_attendence explicitly set to 0 (default ON)
        $employers = User::where(function($q) {
                $q->whereIn('auto_attendence', [1, '1', true])
                  ->orWhereNull('auto_attendence');
            })
            ->where('auto_attendence', '!=', 0)
            ->where('is_active', 1)
            ->where('is_deleted', 0)
            ->get();
        ...
        foreach ($employers as $employer) {
            try {
                ...
                $directlyAddedStaffIds = User::where('added_by', $employer->id)
                    ->orWhere('parent_user_id', $employer->id)
                    ->pluck('id')
                    ->toArray();
