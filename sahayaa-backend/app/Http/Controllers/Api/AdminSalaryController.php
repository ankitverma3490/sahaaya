...
class AdminSalaryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $salaries->getCollection()->transform(function ($salary) {
            if (!$salary->staff && $salary->staff_id) {
                $rawStaff = User::find($salary->staff_id);
                if ($rawStaff) {
                    $salary->setRelation('staff', $rawStaff);
                }
            }
            return $salary;
        });
    ...
    public function store(Request $request)
    {
        ...
        // ✅ Calculate Net Salary (Capped at 0) — deductions include ONLY PF and IT
        $totalEarnings = 
        ...
        // Send notification to staff: salary slip ready + paid notification
        $staff = User::find($request->staff_id);
        $owner = User::find($request->houseowner_id);
        if ($staff) {
            try {
                \App\Services\NotificationService::salarySlip(
                    $staff->id,
                    now()->format('F Y'),
                    number_format($netSalary, 2)
                );
            } catch (\Throwable $e) {
                \Log::warning('salarySlip notification failed: ' . $e->getMessage());
            }
