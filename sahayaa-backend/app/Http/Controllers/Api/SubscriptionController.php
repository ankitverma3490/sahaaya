...
class SubscriptionController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        // Sync active user subscriptions with updated plan limits so mobile app reflects immediately
        if (array_key_exists('job_limit', $data)) {
            SubscriptionUser::where('subscription_id', $id)
                ->where('status', 'active')
                ->update(['job_user_limit' => (float) $data['job_limit']]);
        }
        ...
        public function createSubscriptionOrder(Request $request)
    {
        ...
        // Block only if the SAME plan is already active (allows free -> paid plan switch)
        ...
        try {
            $useWallet = $request->has('use_wallet') ? (bool) $request->use_wallet : false;
            ...
            if($payable_amount == 0){
                // Cancel any other active subscriptions (free -> paid upgrade)
                SubscriptionUser::where('user_id', $user->id)
                    ->where('status', 'active')
                    ->where('id', '!=', $activeSubscription->id ?? 0)
    ...
    public function getCurrentSubscription()
    {
        ...
        if (!$subscription) {
            $freePlan = Subscription::where(function($q) use ($user) {
                    $q->where('role_id', $user->user_role_id)
                      ->orWhere('role_id', 3)
                      ->orWhereNull('role_id');
                })
                ->where(function($q) {
            ...
            if ($freePlan) {
                $subscription = [
                    ...
                    'status' => 'active',
                    'job_user_limit' => (float) ($freePlan->job_limit ?? 0),
    ...
    public function createExtraStaffOrder(Request $request)
    {
        ...
        if (!$activeSubscription) {
            $plan = Subscription::where(function($q) use ($user) {
                    $q->where('role_id', $user->user_role_id)
                      ->orWhere('role_id', 3)
                      ->orWhereNull('role_id');
                })
