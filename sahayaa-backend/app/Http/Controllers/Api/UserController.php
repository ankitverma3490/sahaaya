...
class UserController extends Controller
{
    ...
    public function signUp(Request $request)
    {
        ...
        $user = User::create([
            ...
            'whatsapp_consent' => $request->boolean('whatsapp_consent', false),
        ]);
  ...
  public function saveLastWorkExperience(Request $request)
    {
        try {
            ...
            if ($joinDate) {
                \App\Models\UserWorkInfo::updateOrCreate(
                    ['user_id' => $user->id],
...
public function deleteAcc()
{
    ...
    \DB::transaction(function () use ($user) {
        if ($user->phone_number && !str_contains($user->phone_number, '_deleted_')) {
        ...
        $user->is_active = 0;
        $user->is_job_seeking = 0;
        $user->aadhar_number = null;
        $user->verification_code = null;
        $user->password = null;

        // If staff member belongs to a household/employer, keep is_deleted = 0 so employer sees them in Inactive Staff tab
        if (!empty($user->added_by) || $user->user_role_id == 2 || $user->is_staff_added == 1) {
            $user->is_deleted = 0;
        } else {
            $user->is_deleted = 1;
            $user->deleted_at = now();
        ...
        // Clear private tokens & credentials while preserving basic staff profile info
        \DB::table('personal_access_tokens')->where('tokenable_id', $user->id)->delete();
        \DB::table('notifications')->where('user_id', $user->id)->delete();
        \DB::table('user_device_tokens')->where('user_id', $user->id)->delete();
        \DB::table('kyc_verifications')->where('user_id', $user->id)->delete();
    });
...
\Log::info('Login request', ['data' => $songName]);
    ...
    */
    ...
    // Statuses that mean the account was intentionally deactivated (by self or employer)
    $deactivatedStatuses = ['deactivated', 'self_deactivated', 'inactive'];
    $isDeactivated = ((int) $user->is_active === 0 && in_array($user->status, $deactivatedStatuses));

    // If account has is_active === 0 due to old termination/pause bug (NOT intentional deactivation), auto-fix it
    if ((int) $user->is_active === 0 && !in_array($user->status, $deactivatedStatuses)) {
...
{ 
    ...
    // A live (non-deleted) account already exists for this number?
    // A half-onboarded account should NOT dead-end with "already registered".
    // Instead, send OTP and restart onboarding fresh (role-select + step 1).
    $existing = User::where('phone_number', $request->phone_number)
        ->where('is_deleted', 0)
        ->orderBy('id')
        ->first();

    if ($existing) {
        // Staff onboarding is considered complete only when Aadhaar is verified
        // AND profile step is >= 5 (matches the app's routing logic).
        if ((int) $existing->user_role_id === 2) {
            $profileComplete = $existing->aadhar__verify && (int) $existing->step >= 5;
        } else {
            $profileComplete = (int) $existing->step >= 4;
        }

        // If user already has a role assigned (added via addStaff or fully onboarded),
        // do NOT reset their role — just ask them to login instead.
        if ((int) $existing->user_role_id > 0) {
            return response()->json([
                'status'  => false,
                'message' => 'This phone number is already registered. Please login.',
                'errors'  => [
                    'phone_number' => ['This phone number has already been taken.'],
                ],
            ], 422);
        }

        if (!$profileComplete) {
            // Half-onboarded account — restart onboarding FRESH instead of
            // resuming mid-way. This makes the app land on the role-select
            // page again and start from step 1, exactly like a new signup.
            $otp = rand(100000, 999999);
            $response = $this->sendOtp($to, $otp);
            if (!$response['success']) {
                \Log::warning('signUpCustomer resume SMS failed', ['number' => $to, 'api' => $response['api']]);
            }
            $this->sendWhatsAppOtp($request->phone_number, $otp);
        ...
        // Fully onboarded account already exists — ask the user to log in.
        return response()->json([
            'status'  => false,
            'message' => 'This phone number is already registered. Please login.',
            'errors'  => [
    ...
    // No existing account — create a fresh user
    $otp = rand(100000, 999999);
    $response = $this->sendOtp($to, $otp);
    if (!$response['success']) {
...
public function getProfile(Request $request)
{
    try {
        ...
        $userDetails = User::with(['addresses.householdInformation','addresses.petDetails','petDetails','lastExp','householdInformation','kycInformation','userWorkInfo','reviewsReceived','addedByUser', 'addedByUser.addresses.householdInformation', 'addedByUser.addresses.petDetails',
            ...
            'addedByUser.userWorkInfo', 'assignedJob'])->find($user->id);

        if ($userDetails && ($userDetails->user_role_id == 1 || $userDetails->user_role_id == '1') && !empty($userDetails->added_by) && $userDetails->addedByUser) {
            if (($userDetails->addresses === null || $userDetails->addresses->isEmpty()) && $userDetails->addedByUser->addresses && $userDetails->addedByUser->addresses->isNotEmpty()) {
                $userDetails->setRelation('addresses', $userDetails->addedByUser->addresses);
            }
            if (!$userDetails->householdInformation && $userDetails->addedByUser->householdInformation) {
                $userDetails->setRelation('householdInformation', $userDetails->addedByUser->householdInformation);
            }
        }

        $startDate = \Carbon\Carbon::now()->startOfMonth();
        $endDate   = \Carbon\Carbon::now()->endOfMonth();
        if ($userDetails && $userDetails->userWorkInfo && $userDetails->userWorkInfo->joining_date) {
            $joiningCarbon = \Carbon\Carbon::parse($userDetails->userWorkInfo->joining_date);
    ...
    public function verifyOtp(Request $request)
    {
        ...
        if (!$user && $request->filled('phone_number')) {
            // Prefer users with an assigned role (fully onboarded) over those without
            $user = User::where('phone_number', $request->phone_number)
                ->where('is_deleted', 0)
            ...
            // Fallback: if no user with role found, get the latest one
            if (!$user) {
                $user = User::where('phone_number', $request->phone_number)
                    ->where('is_deleted', 0)
                    ->latest('id')
        ...
        if ((string)$request->otp === (string)$user->verification_code) {
            try {
                ...
                // Auto-activate account when OTP is verified
                $user->update([
                    'is_active' => 1,
                    'status' => in_array($user->status, ['inactive', 'deactivated', 'self_deactivated', 'paused']) ? 'active' : ($user->status ?? 'active'),
                ]);
    ...
    public function aadharVerifyOtp(Request $request)
    {
        try {
            ...
            // Extract mobile / phone number from Aadhaar verification response
            $rawVerify = $verifyResult['raw_data']['data'] ?? $verifyResult['raw_data'] ?? [];

            // Extract and normalize Gender from Aadhaar
            $rawGender = $aadhaarData['gender'] ?? $rawVerify['gender'] ?? $rawVerify['sex'] ?? null;
            if (!empty($rawGender)) {
                $g = strtolower(trim((string)$rawGender));
                if ($g === 'm' || $g === 'male' || str_starts_with($g, 'm')) {
                    $normalizedGender = 'male';
                } elseif ($g === 'f' || $g === 'female' || str_starts_with($g, 'f')) {
                    $normalizedGender = 'female';
                } else {
                    $normalizedGender = 'other';
                }
                $user->gender = $normalizedGender;
                $aadhaarData['gender'] = $normalizedGender;
            }

            // Extract and normalize Date of Birth from Aadhaar
            $rawDob = $aadhaarData['dob'] 
                ?? $rawVerify['dob'] 
                ?? $rawVerify['date_of_birth'] 
                ?? $rawVerify['birthdate'] 
                ?? (!empty($rawVerify['year_of_birth']) ? $rawVerify['year_of_birth'] . '-01-01' : null)
                ?? (!empty($rawVerify['yob']) ? $rawVerify['yob'] . '-01-01' : null)
                ?? null;

            if (!empty($rawDob)) {
                try {
                    $dobStr = trim((string)$rawDob);
                    $dobFormatted = null;
                    if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $dobStr)) {
                        $dobFormatted = $dobStr;
                    } elseif (preg_match('/^\d{2}-\d{2}-\d{4}$/', $dobStr)) {
                        $d = \DateTime::createFromFormat('d-m-Y', $dobStr);
                        if ($d) $dobFormatted = $d->format('Y-m-d');
                    } elseif (preg_match('/^\d{2}\/\d{2}\/\d{4}$/', $dobStr)) {
                        $d = \DateTime::createFromFormat('d/m/Y', $dobStr);
                        if ($d) $dobFormatted = $d->format('Y-m-d');
                    } elseif (preg_match('/^\d{4}\/\d{2}\/\d{2}$/', $dobStr)) {
                        $dobFormatted = str_replace('/', '-', $dobStr);
                    } elseif (preg_match('/^\d{4}$/', $dobStr)) {
                        $dobFormatted = $dobStr . '-01-01';
                    }

                    if ($dobFormatted) {
                        $user->dob = $dobFormatted;
                        $aadhaarData['dob'] = $dobFormatted;
                    }
                } catch (\Exception $e) {
                    \Log::warning('DOB conversion failed: ' . $e->getMessage());
                }
            }

            $aadhaarPhone = $aadhaarData['mobile_number'] 
                ?? $aadhaarData['phone_number'] 
                ?? $aadhaarData['phone'] 
                ?? $aadhaarData['mobile'] 
                ?? $aadhaarData['contact_number'] 
                ?? $rawVerify['mobile_number'] 
                ?? $rawVerify['phone_number'] 
                ?? $rawVerify['phone'] 
                ?? $rawVerify['mobile'] 
                ?? $rawVerify['contact_number'] 
                ?? null;

            if (!empty($aadhaarPhone)) {
                $cleanPhone = preg_replace('/[^0-9]/', '', (string)$aadhaarPhone);
                if (strlen($cleanPhone) > 10 && substr($cleanPhone, 0, 2) === '91') {
                    $cleanPhone = substr($cleanPhone, 2);
                }
                if (strlen($cleanPhone) === 10) {
                    if (empty($user->phone_number) || $user->is_staff_added == 1 || $user->user_role_id == 2) {
                        $user->phone_number = $cleanPhone;
                    }
                    $aadhaarData['mobile_number'] = $cleanPhone;
                }
            }
            ...
            try {
                ...
                $rawData = $rawResponse['data'] ?? $rawResponse ?? [];
                $splitAddress = $rawData['split_address'] ?? $aadhaarData['address'] ?? $rawData['address'] ?? null;

                if (is_string($splitAddress)) {
                    $splitAddress = ['street' => $splitAddress];
                }

                if (is_array($splitAddress) && !empty($splitAddress)) {
                    $streetParts = array_filter([
                        $splitAddress['house'] ?? null,
                        $splitAddress['street'] ?? null,
                        $splitAddress['locality'] ?? null,
                        $splitAddress['po'] ?? null,
                        $splitAddress['landmark'] ?? null,
                        $splitAddress['care_of'] ?? null,
                        $splitAddress['co'] ?? null,
                    ]);
                    $street = implode(', ', $streetParts);
                    $areaLocality = $splitAddress['locality'] ?? $splitAddress['area'] ?? $splitAddress['road'] ?? $splitAddress['subdist'] ?? $splitAddress['po'] ?? null;
                    $city = $splitAddress['dist'] ?? $splitAddress['district'] ?? $splitAddress['vtc'] ?? $splitAddress['city'] ?? null;
                    $state = $splitAddress['state'] ?? null;
                    $pincode = $splitAddress['pincode'] ?? $splitAddress['pc'] ?? null;

                    if (!empty($street) || !empty($city) || !empty($state) || !empty($pincode)) {
                        $existingPermanent = \App\Models\UserAddress::where('user_id', $user->id)
                            ->where('address_type', 'permanent')->first();
                        $existingPresent = \App\Models\UserAddress::where('user_id', $user->id)
                            ->where('address_type', 'present')->first();
                        ...
                        if ($existingPermanent) {
                            $existingPermanent->update($updateData);
                        ...
                        // Also save on user model columns
                        if ($street) $user->permanent_street = $street;
                        if ($areaLocality) $user->permanent_area = $areaLocality;
                        if ($city) $user->permanent_city = $city;
                        if ($state) $user->permanent_state = $state;
                        if ($pincode) $user->permanent_pincode = $pincode;
                        $user->save();
                    }
                }
            ...
            return response()->json([
                ...
                'aadhaar_details' => collect($aadhaarData)->except(['photo'])->toArray(),
            ]);
...
public function updateProfile(Request $request)
{
    try {
        ...
        if ($request->has('addresses')) {
            try {
                \DB::transaction(function () use ($user, $request, &$savedAddressCount, &$savedNestedHousehold, &$savedNestedPets) {
                    $savedAddressCount = 0;
                    $incomingAddressIds = array_filter(array_column($request->addresses, 'id'));
                    $oldAddressIds = $user->addresses()->pluck('id')->toArray();
                    $addressesToDelete = array_diff($oldAddressIds, $incomingAddressIds);
                    ...
                    if (!empty($addressesToDelete)) {
                        try {
                            ...
                            \Log::error('Could not delete address due to constraint: ' . $e->getMessage());
                        }
                    }
                    ...
                    foreach ($request->addresses as $address) {
                        ...
                        if ($hasData) {
                            $safe = array_intersect_key($address, array_flip([
                                'street', 'city', 'state', 'pincode', 'is_primary', 'area_locality', 'google_location', 'address_type'
                            ]));
                            ...
                            if (isset($address['id']) && in_array($address['id'], $oldAddressIds)) {
                                $savedAddress = $user->addresses()->where('id', $address['id'])->first();
                                if ($savedAddress) {
                                    $savedAddress->update($safe);
                                    \App\Models\UserHouseholdInformation::where('user_id', $user->id)
        ...
        if ($isEdit != 1) {
            $finalStep = max((int) $user->step, 1); // Default to current step or 1
            
            // Check if basic info was provided in this request
            if ($request->hasAny(['first_name', 'last_name', 'dob', 'gender', 'name', 'profile_picture'])) {
                $finalStep = max($finalStep, 2);
            }
...
private function saveWorkAndExperience($user, $request, $isEdit)
{
    ...
    if ($request->has('relation')) {
        $data['emergency_contact_relation'] = $workValidated['relation'];
    }
    ...
    if ($request->hasAny($expFields)) {
        ...
        if ($request->has('role')) {
            $roleVal = $expValidated['role'];
            $decoded = json_decode($roleVal, true);
            if (is_array($decoded)) {
                $expData['role'] = implode(', ', array_map('trim', $decoded));
            } else {
                $expData['role'] = $roleVal;
            }
        }
        ...
        $expId = $expValidated['id'] ?? LastWorkExperience::where('user_id', $user->id)->value('id');
        LastWorkExperience::updateOrCreate(
            ['id' => $expId, 'user_id' => $user->id],
            $expData
        );
    }
            ...
            foreach ($request->addresses as $addr) {
                if (!empty(array_filter($addr))) {
                    $newAddress = \App\Models\UserAddress::create([
                        ...
                        'address_type'   => $addr['address_type'] ?? 'present',
                    ]);
...
public function saveAadharAndSendOtp(Request $request)
{
    try {
        ...
        $sendOtpHandler = function($aadharNumber) use ($aadhaarService) {
            return $aadhaarService->sendOtp($aadharNumber);
        };
        ...
        if ($request->is_staff_add == 1) {
            ...
            // If Aadhaar belongs to an existing user:
            // Allow verification if user_id matches OR if existing user is already a staff member (user_role_id == 2)
            $isSameStaff = false;
            if ($existingUser) {
                if ($request->filled('user_id') && $existingUser->id == $request->user_id) {
                    $isSameStaff = true;
                } elseif ($existingUser->user_role_id == 2) {
                    $isSameStaff = true;
                }
            }

            if ($existingUser && !$isSameStaff) {
                if (empty($existingUser->aadhar__verify) || $existingUser->aadhar__verify == 0) {
                    // Previous user never verified this Aadhaar. Free it up.
                    $existingUser->update(['aadhar_number' => null]);
                    $existingUser = null; // Proceed as if not found
                } else {
                    return response()->json([
                        'status' => false,
                        'message' => 'This Aadhaar number is already registered with another account. Each Aadhaar can only be linked to one user.',
                    ], 409);
                }
            }
            ...
            if (!$targetUser) {
                $targetUser = new User();
                $targetUser->name = 'Staff Member';
                $targetUser->is_staff_added = 1;
                $targetUser->step = 4;
                $targetUser->user_role_id = 2;
            }

            $targetUser->aadhar_number = $request->aadhar_number;
            $targetUser->aadhar_reference_id = $otpResult['reference_id'];
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'aadhar_number' => $targetUser->aadhar_number,
        ...
        if ($existingUser) {
            if (empty($existingUser->aadhar__verify) || $existingUser->aadhar__verify == 0) {
                // Previous user (likely an employer stub or failed signup) never verified this Aadhaar. Free it up.
                $existingUser->update(['aadhar_number' => null]);
            } else {
                return response()->json([
                    'status' => false,
    ...
    public function verifyAadharOtp(Request $request)
    {
        try {
            ...
            if ($request->has('aadhar_number') && $request->aadhar_number) {
                $user->aadhar_number = $request->aadhar_number;
                $user->save();
            }
            ...
            // Auto-fill details and address from Aadhaar verification response
            $streetStr = null;
            $localityStr = null;
            $cityStr = null;
            $stateStr = null;
            ...
            if (!empty($verifyResult['aadhaar_data'])) {
                ...
                $aadhaarName = $details['name'] ?? $details['full_name'] ?? null;
                
                if ($aadhaarName) {
                    $user->name = $aadhaarName;
                    $nameParts = explode(' ', trim($aadhaarName));
                    $user->first_name = $nameParts[0];
                    $user->last_name = count($nameParts) > 1 ? implode(' ', array_slice($nameParts, 1)) : '';
                }
                
                if (!empty($details['gender'])) $user->gender = strtolower($details['gender']);
                if (!empty($details['dob'])) $user->dob = $details['dob'];

                // Extract mobile / phone number from Aadhaar verification response
                $rawVerify = $verifyResult['raw_data']['data'] ?? $verifyResult['raw_data'] ?? [];
                $aadhaarPhone = $details['mobile_number'] 
                ...
                if (!empty($aadhaarPhone)) {
                    ...
                    if (strlen($cleanPhone) === 10 && empty($user->phone_number)) {
                        $user->phone_number = $cleanPhone;
                    }
                }
                ...
                if (!empty($details['address'])) {
                    $rawAddr = $details['address'];
                    if (is_array($rawAddr)) {
                        $streetParts = array_filter([
                            $rawAddr['house'] ?? null,
                            $rawAddr['street'] ?? null,
                            $rawAddr['landmark'] ?? null,
                            $rawAddr['care_of'] ?? null,
                            $rawAddr['co'] ?? null,
                        ]);
                        $streetStr = !empty($streetParts) ? implode(', ', $streetParts) : null;
                        $localityStr = $rawAddr['locality'] ?? $rawAddr['area'] ?? $rawAddr['road'] ?? $rawAddr['subdist'] ?? $rawAddr['po'] ?? null;
                        $cityStr = $rawAddr['district'] ?? $rawAddr['city'] ?? $rawAddr['vtc'] ?? null;
                        $stateStr = $rawAddr['state'] ?? null;
                        $pincodeStr = $rawAddr['pincode'] ?? $rawAddr['zip'] ?? null;
                    } elseif (is_string($rawAddr)) {
                        $streetStr = $rawAddr;
                    }
                }
            }

            // Save permanent address on user columns if found
            if ($streetStr || $cityStr || $stateStr || $pincodeStr) {
                if ($streetStr) $user->permanent_street = $streetStr;
                if ($localityStr) $user->permanent_area = $localityStr;
                if ($cityStr) $user->permanent_city = $cityStr;
                if ($stateStr) $user->permanent_state = $stateStr;
                if ($pincodeStr) $user->permanent_pincode = $pincodeStr;

                // Also save / update UserAddress record with type 'permanent'
                $existingPerm = \App\Models\UserAddress::where('user_id', $user->id)
                    ->where(function($q) {
                        $q->where('address_type', 'permanent')->orWhere('name', 'Permanent Address');
                    })
                    ->first();

                if ($existingPerm) {
                    $existingPerm->update([
                        'street'        => $streetStr ?: $existingPerm->street,
                        'area_locality' => $localityStr ?: $existingPerm->area_locality,
                        'city'          => $cityStr ?: $existingPerm->city,
                        'state'         => $stateStr ?: $existingPerm->state,
                        'pincode'       => $pincodeStr ?: $existingPerm->pincode,
                    ]);
                } else {
            ...
            $userWithRelations = User::with(['addresses', 'userWorkInfo'])->find($user->id);
...
public function addressUpdate(Request $request)
{
    ...
    for ($i = 0; $i < $count; $i++) {
        $isPrimary = isset($data['is_primary'][$i]) ? (bool)$data['is_primary'][$i] : false;
        $addresses[] = [
            ...
            'address_type' => $isPrimary ? 'permanent' : 'present',
            'id' => $data['id'][$i] ?? null,
...
public function deleteSelfAccount(Request $request)
{
    try {
        ...
        try {
            // --- Remove personal / profile (PII) records ---
            // Tokens (all devices + API tokens)
            ...
            \DB::table('userdevicetokens')->where('user_id', $userId)->delete();
            \DB::table('user_addresses')->where('user_id', $userId)->delete();
            \DB::table('user_pet_details')->where('user_id', $userId)->delete();
            \DB::table('user_household_informations')->where('user_id', $userId)->delete();
            \DB::table('user_work_infos')->where('user_id', $userId)->delete();
            \DB::table('kyc_verifications')->where('user_id', $userId)->delete();
            \DB::table('last_work_experiences')->where('user_id', $userId)->delete();
            \DB::table('bank_accounts')->where('user_id', $userId)->delete();
            \DB::table('user_policy_acceptances')->where('user_id', $userId)->delete();
            \DB::table('legal_consents')
                ->where(function ($q) use ($userId, $originalPhone) {
                    $q->where('user_id', $userId);
                    if (!empty($originalPhone)) {
                        $q->orWhere('phone_number', $originalPhone);
                    }
                })
                ->delete();
            \DB::table('user_other_images')->where('user_id', $userId)->delete();
            \DB::table('user_social_links')->where('user_id', $userId)->delete();
            \DB::table('wishlists')->where('user_id', $userId)->delete();
            \DB::table('subscription_users')->where('user_id', $userId)->delete();
            // Reviews written by or written about this user
            \DB::table('reviews')->where('given_by_id', $userId)->where('given_by_type', 'user')->delete();
            \DB::table('reviews')->where('received_by_id', $userId)->where('received_by_type', 'user')->delete();

            // --- Clear references pointing back to this user ---
            \DB::table('users')->where('added_by', $userId)->update(['added_by' => null]);
            \DB::table('users')->where('parent_user_id', $userId)->update(['parent_user_id' => null]);
            \DB::table('users')->where('referred_by', $userId)->update(['referred_by' => null]);
            \DB::table('attendance')->where('processed_by', $userId)->update(['processed_by' => null]);
            \DB::table('terminations')->where('approved_by', $userId)->update(['approved_by' => null]);
            \DB::table('salary_payouts')->where('requested_by', $userId)->update(['requested_by' => null]);
            \DB::table('transactions')->where('created_by', $userId)->update(['created_by' => null]);

            // Business / transactional records are intentionally KEPT (hybrid delete):
            // attendance, salaries, staff_advances, advance_transactions, job_applications,
            // quit_jobs, leave_requests, terminations, salary_payouts, transactions, payments,
            // tickets, ticket_comments, jobs, bookings. They still reference the soft-deleted
            // user row below, so employer-side history stays intact.
...
/**
 * Deactivate staff account (no data deletion — just sets inactive)
 */
public function deactivateSelfAccount(Request $request)
{
    try {
        $user = Auth::guard('api')->user();
...
public function deleteUserByAdmin(Request $request)
{
    try {
        ...
        \DB::transaction(function () use ($userToDelete, $adminUser) {
            ...
            $userToDelete->deleted_by = $adminUser->id;
            $userToDelete->save();
        });
        ...
        try {
            ...
            try {
                \App\Models\Attendance::firstOrCreate(
                    [
                        'staff_id' => $staff->id,
                        'date' => \Carbon\Carbon::now('Asia/Kolkata')->toDateString(),
                    ],
                    [
                        'check_in_time' => '07:00:00',
                        'status' => 'present',
                        'description' => 'Default present on hiring',
                        'processed_by' => $authUser->id ?? 1,
                    ]
                );
            } catch (\Throwable $th) {
                \Log::warning('Default attendance creation failed: ' . $th->getMessage());
            }
        ...
        try {
            if ($staff) {
                ...
                $workInfo = UserWorkInfo::create([
                    ...
                    'preferred_work_location' => $request->preferred_work_location ?? null,
...
private function updateExistingStaff(User $existingUser, Request $request)
{
    ...
    // Restore soft-deleted record so Laravel's SoftDeletes scope re-includes it
...
// dd($existingUser);
        ...
        try {
            ...
            $existingAddress = UserAddress::where('user_id', $existingUser->id)
                ->where('address_type', 'present')->first();

            if (!$existingAddress) {
                $existingAddress = UserAddress::where('user_id', $existingUser->id)
                    ->where(function ($q) {
                        $q->where('address_type', null)
                            ->orWhere('address_type', 'present')
                            ->orWhere('address_type', '');
                    })
                    ->orderBy('is_primary', 'desc')
                    ->first();
            }
            ...
            // Update or create permanent address if provided
            try {
                if ($request->perm_street || $request->perm_city || $request->perm_state || $request->perm_pincode) {
                    $existingPerm = UserAddress::where('user_id', $existingUser->id)
                        ->where('address_type', 'permanent')->first();
                    ...
                    \Log::info('Permanent address processed successfully', [
                        'action' => $logAction,
                        'user_id' => $existingUser->id,
                        'timestamp' => now()->toDateTimeString()
                    ]);
...
// dd($existingUser);
        // Fire-and-forget: notify re-hired staff via WhatsApp
        try {
            \App\Services\NotificationService::staffAdded($existingUser->id, $authUser->name);
        } catch (\Throwable $e) {
            \Log::warning('Re-hired staff notification failed', [
    ...
    public function getStaffList(Request $request)
    {
        try {
            ...
            // Resolve the primary owner ID so family members (role=1) see the same staff
            $effectiveOwnerId = $user->getEffectiveOwnerId();
            ...
            $hiredStaffIds = JobApplication::whereIn('application_status', $hiredStatuses)
                ->whereHas('job', function($query) use ($effectiveOwnerId) {
                    $query->where('created_by', $effectiveOwnerId);
                })
                ->pluck('user_id')
                ->toArray();
            ...
            $directlyAddedStaffIds = User::where('user_role_id', 2)
                ->where('added_by', $effectiveOwnerId)
                ->pluck('id')
                ->toArray();
            ...
            $staff->getCollection()->transform(function ($item) use ($user) {
                if ($item->is_active == 0 || $item->status === 'inactive' || $item->status === 'terminated') {
    ...
    public function getStaffDetails($id)
    {
        try {
            ...
            // Resolve effective owner so family members see the same staff
            $effectiveOwnerId = $user->getEffectiveOwnerId();
            ...
            if ($user) {
                ...
                if (in_array($id, $allStaffIds)) {
                    ...
                    if ($staff->added_by != $user->id) {
                        // status is fetched directly from users table now
                    }
                }
   ...
   public function updateStaff(Request $request, $id)
{
    ...
    try {
        ...
        try {
            ...
            $directlyAddedStaffIds = User::where('added_by', $authUser->id)
                ->pluck('id')
                ->toArray();

            $allStaffIds = array_unique(array_merge($hiredStaffIds, $directlyAddedStaffIds, [$authUser->id]));

            $staff = User::where('id', $id)
                ->where(function($query) use ($authUser, $allStaffIds) {
                    $query->whereIn('id', $allStaffIds)
                          ->orWhere('added_by', $authUser->id)
    ...
    public function getMyWork(Request $request)
    {
        try {
            ...
            // Check if staff has been terminated or is inactive
            $isTerminated = \App\Models\Termination::where('user_id', $user->id)->exists();
            $isInactive = ($userDetails->is_active == 0 || $userDetails->status === 'inactive' || $userDetails->is_staff_added == 0);

            if ($isTerminated || $isInactive) {
                $userDetails->added_by = null;
                $userDetails->setRelation('addedByUser', null);
            }
    ...
    public function applyReferralCode(Request $request, ReferralPointService $referralPoints)
    {
        ...
        try {
            ...
            $result = DB::transaction(function () use ($authenticatedUser, $referralCode, $referralPoints) {
                ...
                // Extra duplicate guard: prevent double credit even if referred_by wasn't saved previously
                ...
                // Referrer gets referral_earnings (they must click 'Redeem' in the app to move it to wallet)
                $referrer->increment('referral_earnings', $rewardAmount);

                // Referee (New User) also gets referral_earnings
                $newUserBonusSetting = setting('referral_new_user_bonus');
                $newUserBonus = (float) ($newUserBonusSetting['value'] ?? 10);
                
                $user->increment('referral_earnings', $newUserBonus);

                ReferralReward::create([
                    'referrer_id' => $referrer->id,
                    'referred_id' => $user->id,
                    'reward_amount' => $rewardAmount,
                    'new_user_reward_amount' => $newUserBonus,
                    'reward_type' => 'signup',
                    'is_credited' => true,
                    'credited_at' => now(),
                ]);
