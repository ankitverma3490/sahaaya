...
class AdvanceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Notify staff with employer name via In-app + Push + WhatsApp + SMS
            $employerName = trim($employer->first_name . ' ' . $employer->last_name) ?: ($employer->name ?: 'Your employer');
            $advanceMsg = 'You have received an advance salary of ₹' . number_format($request->amount, 2) .
            ...
            // Direct WhatsApp message send fallback
            try {
                $staffUser = User::find($request->staff_id);
                if ($staffUser && !empty($staffUser->phone_number)) {
                    $wa = new \App\Services\WhatsAppService();
                    if ($wa->isConfigured()) {
                        $wa->sendTextMessage($staffUser->phone_number, $advanceMsg);
                    }
                }
            } catch (\Throwable $waErr) {
                \Log::warning('Advance WhatsApp direct send failed: ' . $waErr->getMessage());
            }
