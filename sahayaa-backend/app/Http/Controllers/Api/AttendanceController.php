...
class AttendanceController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            ...
            // Notify staff via WhatsApp when a leave is marked (absent)
            if ($status === 'absent') {
                try {
                    \App\Services\NotificationService::leaveMarked($staffId, $date, $status);
                } catch (\Throwable $e) {
                    \Log::warning('leaveMarked notification failed: ' . $e->getMessage());
                }
            }
