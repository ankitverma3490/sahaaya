...
class NotificationService extends Controller
{
    ...
    protected static function sendWhatsApp($userId, $message)
    {
        try {
            ...
            if ($user && !empty($user->phone_number)) {
                self::getWhatsApp()->sendTextMessage($user->phone_number, $message);
            }
        } catch (\Throwable $e) {
    ...
    protected static function sendWhatsAppTemplate($userId, $methodName, $params = [])
    {
        try {
            $user = User::find($userId);
            if (!$user || empty($user->phone_number) || !$user->whatsapp_consent) return;
    ...
    /**
     * Quit Job Approved - notify staff
     */
    public static function quitApproved($staffId, $ownerName, $jobTitle)
    {
        $message = "{$ownerName} has approved your quit request for \"{$jobTitle}\". You have been removed from this job.";
        try {
            self::send($staffId, 'Quit Request Approved', $message, 'job_quit_approved');
        } catch (\Throwable $e) {
            \Log::warning('quitApproved send failed: ' . $e->getMessage());
        }
        try {
            self::sendWhatsAppTemplate($staffId, 'quitApproved', [$ownerName, $jobTitle]);
        } catch (\Throwable $e) {
            \Log::warning('quitApproved WhatsApp failed: ' . $e->getMessage());
        }
    }

    /**
     * Quit Job Rejected - notify staff
     */
    public static function quitRejected($staffId, $ownerName, $jobTitle)
    {
        $message = "{$ownerName} has rejected your quit request for \"{$jobTitle}\". Please continue working.";
        try {
            self::send($staffId, 'Quit Request Rejected', $message, 'job_quit_rejected');
        } catch (\Throwable $e) {
            \Log::warning('quitRejected send failed: ' . $e->getMessage());
        }
        try {
            self::sendWhatsAppTemplate($staffId, 'quitRejected', [$ownerName, $jobTitle]);
        } catch (\Throwable $e) {
            \Log::warning('quitRejected WhatsApp failed: ' . $e->getMessage());
        }
    }
