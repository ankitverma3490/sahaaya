...
Route::get('/proof/aadhaar-response', function () {
    ...
    return response()->json([
        'proof' => 'This is the RAW response from Digio API (not our app)',
        'aadhaar_tested' => $aadhaar,
        'timestamp' => now()->toDateTimeString(),
        'digio_raw_response' => $result,
        'note' => 'The message comes from Digio, not from our code'
    ]);
});
    ...
    try {
        ...
        foreach ($users as $user) {
            try {
                ...
                $autoEnabled = ($employer->auto_attendence !== 0 && $employer->auto_attendence !== "0" && $employer->auto_attendence !== false);
                if (!$autoEnabled) {
...
Route::prefix('/admin')->middleware('auth:api')->group(function () {
    ...
    Route::get('/settings', [SettingController::class, 'getAllSettings']);
        ...
        // ── Training Video Management (Admin) ─────────────────────
        Route::get('/training-videos', [\App\Http\Controllers\Api\TrainingVideoController::class, 'adminIndex']);
        Route::post('/training-videos', [\App\Http\Controllers\Api\TrainingVideoController::class, 'store']);
        Route::post('/training-videos/{id}', [\App\Http\Controllers\Api\TrainingVideoController::class, 'update']);
        Route::delete('/training-videos/{id}', [\App\Http\Controllers\Api\TrainingVideoController::class, 'destroy']);
...
Route::group(['middleware' => 'auth:api'], function() {
    ...
    Route::get('/my-posted-jobs', [JobController::class, 'myJobs']);
    Route::delete('/my-posted-jobs/{id}', [JobController::class, 'deleteMyJob']);
    ...
    Route::prefix('staff')->group(function () {
        ...
        Route::post('/{id}/reactivate', [StaffController::class, 'reactivateStaff']);
    ...
    Route::post('user/deactivate-self', [UserController::class, 'deactivateSelfAccount']);
    Route::post('user/activate-account', [UserController::class, 'activateAccount']);
    ...
    Route::get('/quit-job-list-owner', [JobApplicationController::class, 'listQuitJobsForOwner']);
    Route::post('/quit-job-request/{id}/approve', [JobApplicationController::class, 'approveQuitJob']);
    Route::post('/quit-job-request/{id}/reject', [JobApplicationController::class, 'rejectQuitJob']);
    ...
    // Salary slip (monthly, generated per closing date)
    Route::get('/salary/slip/{id}', [SalaryController::class, 'getSalarySlip']);
...
// ── Training Videos (public for mobile app, no auth required) ─────
Route::get('/training-videos', [\App\Http\Controllers\Api\TrainingVideoController::class, 'index']);
